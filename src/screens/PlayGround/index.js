...
import { AsyncStorage, Platform, StyleSheet, Text, View, TextInput, FlatList, TouchableOpacity, Keyboard, ActivityIndicator } from 'react-native';
      ...
      return (
        <View style={{ flex: 1 }}>
          <LinearGradient colors={['#0ba19e', '#1A2980']} style={{ flex: 1 }}>
            ...
            <TouchableOpacity style={{ marginTop: -30 }} onPress={() => { this.RBSheet.open() }}>
              <View style={{ alignItems: 'center', paddingBottom: 25 }}>
                <Text style={(this.state.focused) ? { display: 'none' } : { color: '#f0f0f0', fontSize: 25, fontWeight: 'bold' }}>Lihat Soal</Text>
              </View>
            </TouchableOpacity>
            <Button buttonStyle={(this.state.focused) ? { display: 'none' } : ''} title="Selesai cuy" type="outline" onPress={() => this.handleSubmit()} />
          </LinearGradient>
        </View>
      )
    }
